...
APP_SLUG="agentx-admin"
BINARY_NAME="agentx-admin"
ALIAS_NAME="axa"
...
case "$os_name/$arch_name" in
  ...
  *)
    echo "agentx-admin installer does not support ${os_name}/${arch_name}." >&2
    exit 1
    ;;
esac
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "agentx-admin installer requires $1." >&2
    exit 1
  fi
}
...
sha512_file() {
  ...
  echo "agentx-admin installer requires sha512sum or shasum." >&2
  exit 1
}

tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/agentx-admin-install.XXXXXX")"
cleanup() {
  rm -rf "$tmp_dir"
}
trap cleanup EXIT
...
if ! fetch_from_sources "$APP_SLUG/latest.json" "$latest_path"; then
  echo "Failed to download agentx-admin latest manifest." >&2
  exit 1
fi
...
if [ "$latest_version" = "" ] || [ "$manifest_path" = "" ]; then
  echo "agentx-admin latest manifest is missing required fields." >&2
  exit 1
fi
...
if [ ! -f "$extract_dir/$BINARY_NAME" ]; then
  echo "agentx-admin archive does not contain $BINARY_NAME." >&2
  exit 1
fi
...
echo "agentx-admin ${latest_version} installed at ${target}."
